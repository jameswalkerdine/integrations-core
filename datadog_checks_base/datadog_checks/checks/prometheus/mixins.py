...
class PrometheusScraperMixin(object):
    ...
    def create_mixin_configuration(self, instance=None):

        # We can choose to create a default mixing configuration for a blank instance
        if instance is None:
            instance = {}
    ...
    def _submit(self, metric, message, scraper_config, hostname=None):
        """
        ...
        """
        metric_name = '{}.{}'.format(scraper_config['NAMESPACE'], metric)
    ...
    def _submit_gauges_from_summary(self, metric_name, metric, scraper_config, hostname=None):
        ...
        if self._is_value_valid(val):
            ...
            self.gauge('{}.count'.format(metric_name), val, tags=tags, hostname=hostname)
        else:
        ...
        if self._is_value_valid(val):
            ...
            self.gauge('{}.sum'.format(metric_name), val, tags=tags, hostname=hostname)
        else:
        ...
        for quantile in getattr(metric, self.METRIC_TYPES[2]).quantile:
            ...
            if self._is_value_valid(val):
                ...
                self.gauge('{}.quantile'.format(metric_name), val, tags=tags, hostname=hostname)
            else:
    ...
    def set_prometheus_timeout(self, instance, default_value=10):
        ...
        #scraper_config['prometheus_timeout'] = instance.get('prometheus_timeout', default_value)
        pass
